...
int client() {
 ...
 limpartela();
  int cont1=0,cont2=0,rep = 0;
	...
	char ipcont[20];
  char contusuario[20];
	arqlogin = fopen("Listadepessoas.txt","r");
    ...
    if (arqlogin == NULL){
      ...
      do {
        ...
        while(fscanf(arqlogin,"%s %s %s",&contato.nome,&contato.ip)!=EOF){
            if(strcmp(conversa,contato.nome) == 0){
                cont2=1;
		            strcpy(ipcont,contato.ip);
                fclose(arqlogin);
